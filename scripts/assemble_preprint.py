...
def build_figures_block() -> str:
    ...
    return (
        ...
        "Stratum 2 (n = 52; 43 splice-affecting, 9 no-effect) for AlphaGenome "
        "under Rule A (composite; solid blue) and Rule B (per-junction "
        "lung/bronchial cell context; dashed orange), SpliceAI 1.3.1 (solid "
        "red), and Pangolin (solid green). Shaded bands are 10,000-iteration "
        ...
        "Rule B's single AUC loss is visible as a small dip near FPR ≈ 0.33 "
        "(one splice-affecting variant misranked below three no-effect "
        "variants whose closest in-context canonical junctions lie 883, 884, "
        "and 2 892 base pairs from the variant respectively; see Results "
        "— Divergence between scoring rules).\n\n"
        ...
        "transcript on Stratum 1 (n = 8) for AlphaGenome under Rule A "
        "(top-left, blue) and Rule B (top-right, orange), SpliceAI "
        "(bottom-left, red), and Pangolin (bottom-right, green). Solid lines "
        ...
        "normal-transcript fraction) is met by both AlphaGenome rules and "
        "both comparators. The Rule B panel shows the wild-type control "
        "(100% normally spliced) at score ≈ 0.5 rather than at the "
        "near-zero position it occupies under Rule A; its closest "
        "lung/bronchial in-context canonical junction is 2 892 base pairs "
        "from the variant.\n"
    )
