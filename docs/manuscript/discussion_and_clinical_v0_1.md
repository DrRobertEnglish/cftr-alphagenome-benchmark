...
On a frozen, pre-registered CFTR splice benchmark, AlphaGenome achieved perfect binary discrimination on Stratum 2 (ROC-AUC 1.000) but was not statistically superior to either SpliceAI 1.3.1 (AUC 0.982) or Pangolin (AUC 0.984) under the pre-specified paired DeLong test. The pre-registered non-inferiority claim (δ = 0.05) held against both comparators. On the small quantitative Stratum 1 (n = 8), AlphaGenome's Spearman rank correlation with the fraction of normally spliced transcript was approximately twice the magnitude of the two comparators (ρ = −0.66 vs −0.32 and −0.29), in the pre-specified negative direction. All three predictors comfortably exceeded the pre-registered distance-to-canonical-splice-site floor (AUC 0.683).

### Interpretation

The Stratum 2 result reflects a well-recognised ceiling effect: CFTR splice variants in Joynt et al. and comparable curated series are heavily enriched for canonical splice-site disruptions, which every modern sequence-based predictor identifies with near-perfect accuracy. Under these conditions, discrimination benchmarks tend to saturate, and small differences in point-estimate AUC do not translate into statistically detectable differences under paired testing at n = 52. The clean sweep by AlphaGenome (AUC 1.000) is therefore consistent with, but not proof of, superior binary discrimination on canonical CFTR splice variation.

Stratum 1 tells a different story. Discrimination is not the question — every variant in this stratum is known to alter splicing to some degree. The question is quantitative fidelity: does a higher predictor score track a lower fraction of normally spliced transcript? AlphaGenome's Spearman ρ of −0.66 is a stronger monotone relationship than either comparator, but the eight-variant sample size gives a wide bootstrap 95% confidence interval that includes zero. We report this as a descriptive finding per the frozen protocol and do not draw an inferential conclusion.

Taken together, the results are consistent with AlphaGenome offering broadly comparable binary performance to established CFTR splice predictors on canonical splice-site variation, with a potentially stronger relationship to quantitative splicing outcome that warrants confirmation in larger quantitative datasets.
