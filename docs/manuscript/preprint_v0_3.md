...
*Preprint version 0.3.1 — 22 September 2026*
...
**Background.** Eligibility for CFTR modulator therapy increasingly depends on the fraction of residual normal transcript a splice variant preserves, yet no genomic foundation model has been benchmarked externally against quantitative *CFTR* splicing measurements under a pre-registered protocol. AlphaGenome was released in 2025 with strong vendor-reported metrics on aggregate splicing benchmarks; independent external evaluation on a clinically-consequential locus has not previously been reported.

**Methods.** We pre-registered a frozen benchmark of 60 *CFTR* variants with published experimental splicing measurements ([osf.io/5hvgf](https://osf.io/5hvgf/) [1], DOI [10.17605/OSF.IO/6PGX8](https://doi.org/10.17605/OSF.IO/6PGX8) [2]) on 13 September 2026. The variant list, ground-truth labels, statistical decision rules, and bootstrap seed were locked before any predictor was scored. Three predictors — AlphaGenome (index model), SpliceAI 1.3.1 (comparator 1), and Pangolin (comparator 2) — were scored on the same GRCh38 chr7 FASTA and the same VCF. After registration, AlphaGenome was scored under two pre-declared rules in parallel (protocol Amendment #2): the DeepMind-recommended merged splicing composite (Rule A) and the OSF-preregistered per-junction score in a lung/bronchial cell context (Rule B). The primary hypothesis (H1) was paired DeLong ROC-AUC superiority of AlphaGenome over SpliceAI on Stratum 2 binary classification (n = 52); the secondary (H2) was non-inferiority to Pangolin with margin δ = 0.05; the descriptive (H3) was Spearman rank correlation on the eight quantitative Stratum 1 variants.

**Results.** AlphaGenome was non-inferior to both SpliceAI and Pangolin on Stratum 2 under both scoring rules (all lower CI bounds on paired ΔAUC well above the pre-specified −0.05 margin), but the pre-registered superiority test did not clear its threshold under either rule (ΔAUC vs SpliceAI: +0.018 under Rule A, p = 0.231; +0.010 under Rule B, p = 0.468). All three predictors converged to Stratum 2 ROC-AUC ≈ 0.98–1.00 with the paired AUC differences sitting inside their SE bands. On Stratum 1 (n = 8) all point estimates matched the pre-specified negative direction (Rule A ρ = −0.659; Rule B ρ = −0.287; SpliceAI ρ = −0.289; Pangolin ρ = −0.323), with wide bootstrap 95% CIs. A pre-declared analysis of the between-rule divergence localises Rule B's failure mode to variants where no annotated splice junction inside the pre-declared cell-context ontology set lies within a few hundred base pairs of the variant: the closest-junction fallback then selects a distant junction whose per-junction magnitude is not a reliable measure of variant impact, while Rule A's cross-track aggregation launders this out.

**Conclusion.** Under an external pre-registered CFTR benchmark AlphaGenome matches but does not exceed two established sequence-based splice predictors, and all three converge to a shared ceiling — a pattern consistent with the compression previously observed between vendor-reported and external metrics for SpliceAI and Pangolin. Reported foundation-model performance on splicing is materially sensitive to the choice of scoring rule; a per-junction cell-context-restricted score becomes unreliable outside annotated junction neighbourhoods, whereas a cross-track composite is more robust. The frozen benchmark, ground-truth tables, statistical code, and per-variant predictor outputs under both rules are released so that subsequently-published splice-effect predictors can be evaluated on the same inputs and pre-registered tests without requiring a new pre-registration.
...
**Keywords.** *CFTR*; splicing; benchmark; AlphaGenome; SpliceAI; Pangolin; pre-registration; cystic fibrosis; scoring rules; cross-tissue aggregation; genomic foundation models; external evaluation.
...
AlphaGenome was non-inferior to both comparators at the pre-registered non-inferiority margin under both scoring rules (Table 1). Against SpliceAI, the lower bound of the 95% CI on the paired ΔAUC was −0.0115 under Rule A and −0.0176 under Rule B — both well inside the pre-specified δ = −0.05. Against Pangolin, AlphaGenome AUC was 1.0000 (Rule A) or 0.9922 (Rule B) versus Pangolin's 0.9845; the ΔAUC lower CI bound was −0.0124 under Rule A and −0.0078 under Rule B, both clearing the non-inferiority margin. All four comparisons therefore support the pre-registered non-inferiority claim, and the finding is robust across scoring rules.

### H3 — Descriptive analysis of quantitative splicing (Spearman rank correlation, Stratum 1)

On the eight quantitative variants, the two scoring rules gave point estimates of the same sign but materially different magnitudes (Figure 2; Table 1). Rule A produced a Spearman rank correlation of ρ = −0.659 with the fraction of normally spliced transcript (10,000-iteration bootstrap 95% CI [−0.98, +0.10]). Rule B produced ρ = −0.287 (95% CI [−0.85, +0.61]). Both estimates matched the pre-specified negative direction. For reference, SpliceAI and Pangolin produced ρ = −0.289 (95% CI [−0.92, +0.57]) and ρ = −0.323 (95% CI [−0.98, +0.67]) respectively (Figure 2, centre and right panels). No formal significance claim is made at this sample size (n = 8); H3 is descriptive per the frozen protocol. Under Rule A, AlphaGenome's monotone tracking of residual splicing is approximately twice the magnitude of the comparators; under Rule B, it is comparable to the comparators (SpliceAI ρ = −0.289; Pangolin ρ = −0.323). The bootstrap confidence intervals cross zero under both rules, and the point estimates are not statistically distinguishable from one another at this sample size.
...
| H2 vs Pangolin | ΔAUC (95% CI); p | +0.0155 (−0.0124 to +0.0434); 0.275 | +0.0078 (−0.0078 to +0.0233); 0.328 | — |
| H2 decision | Non-inferiority (δ = 0.05) | met | met | — |
| H3 Stratum 1 (n = 8) | Spearman ρ (bootstrap 95% CI) | −0.659 (−0.98 to +0.10) | −0.287 (−0.85 to +0.61) | SpliceAI −0.289 (−0.92 to +0.57); Pangolin −0.323 (−0.98 to +0.67) |
| H3 direction | Negative point estimate | met | met | met |

### Table 2 — Vendor-reported versus external splicing metrics

For context, the vendor-reported and independent external splicing metrics for the three predictors evaluated here are compiled below. Numbers are taken from the primary sources cited; the units differ between rows and are therefore not directly comparable across models, but the *within-model* compression from vendor benchmark to external evaluation is informative and forms the basis of the Discussion subsection "Vendor-versus-external compression across splice predictors."

| Predictor | Vendor-reported headline (task, metric) | External evaluation (task, metric) | Compression |
|---|---|---|---|
| SpliceAI 1.3.1 | Top-k accuracy 0.95 on canonical splice-junction prediction in held-out GENCODE test chromosomes; SpliceAI-10k PR-AUC ≈ 0.98 ([Jaganathan et al., 2019, *Cell*](https://doi.org/10.1016/j.cell.2018.12.015) [18]) | MFASS variant-effect auPRC 0.49 ([Avsec et al., 2026, *Nature*](https://www.nature.com/articles/s41586-025-10014-0) [23]); Cassinari et al. clinical cohort AUROC 0.894 ([Cassinari et al., 2025](https://pure.eur.nl/ws/files/218854365/1-s2.0-S2666247725001241-main.pdf) [24]) | 0.95 top-k → ≈ 0.49 auPRC on external MPRA |
| Pangolin | Top-1 accuracy 0.79 and AUPRC 0.85 across tissues on held-out chromosome splice-site prediction ([Zeng and Li, 2022, *Genome Biology*](https://link.springer.com/article/10.1186/s13059-022-02664-4) [11]) | MFASS auPRC 0.54; ClinVar splice-region variant auPRC 0.55 ([Avsec et al., 2026, *Nature*](https://www.nature.com/articles/s41586-025-10014-0) [23]) | 0.85 AUPRC → ≈ 0.54 auPRC on external functional assay |
| AlphaGenome | SOTA on 25/26 vendor variant-effect benchmarks; ClinVar splicing auPRC 0.51–0.66 across subsets; MFASS auPRC 0.51 ([Avsec et al., 2026, *Nature*](https://www.nature.com/articles/s41586-025-10014-0) [23]) | Lu et al. independent clinical benchmark AUROC 0.592 with the vendor cross-modality scorer; 0.779 with the best splicing-specific scorer ([Holgersen review of Lu et al., 2025](http://erle.bio/blog/2025-11-08-how-robust-are-clinvar-variant-subsets/) [25]) | Vendor auPRC ≈ 0.57–0.66 → external AUROC 0.59 (cross-modality) / 0.78 (best rule) |
| **This benchmark (all three, n = 52 CFTR Stratum 2)** | — | Rule A ROC-AUC 1.000 (AlphaGenome), Rule B 0.9922; SpliceAI 0.9819; Pangolin 0.9845. Superiority not met under either rule; non-inferiority met against both comparators. | All three predictors converge to ≈ 0.98–1.00 AUC on the frozen *CFTR* benchmark |
...
Stratum 1 tells a different and complementary story. Discrimination is not the question — every variant in this stratum is known to alter splicing to some degree. The question is quantitative fidelity: does a higher predictor score track a lower fraction of normally spliced transcript? Under Rule A, AlphaGenome's Spearman ρ of −0.66 is roughly twice the magnitude of either comparator (SpliceAI −0.289; Pangolin −0.323) in the pre-specified negative direction. Under Rule B, ρ is −0.29, comparable to the comparators. The eight-variant sample size gives wide bootstrap 95% confidence intervals that include zero under all four measurements, and the rule-specific point estimates are not statistically distinguishable from one another at this sample size. We report this as a descriptive finding per the frozen protocol and do not draw an inferential conclusion.
...
#### Vendor-versus-external compression across splice predictors

The convergence of AlphaGenome, SpliceAI, and Pangolin to ROC-AUC ≈ 0.98–1.00 on this external *CFTR* benchmark (Table 2) is best read against the substantial spread in their vendor-reported metrics on their own benchmarks. SpliceAI's original 95% top-k accuracy on canonical splice-junction identification in held-out GENCODE chromosomes ([Jaganathan et al., 2019, *Cell*](https://doi.org/10.1016/j.cell.2018.12.015) [18]) compresses to auPRC ≈ 0.49 on the MFASS massively-parallel splicing assay ([reported in Avsec et al., 2026, *Nature*](https://www.nature.com/articles/s41586-025-10014-0) [23]). Pangolin, benchmarked internally on quantitative splice-site usage across four tissues in four species, reported top-1 accuracy 0.79 and AUPRC 0.85 ([Zeng and Li, 2022, *Genome Biology*](https://link.springer.com/article/10.1186/s13059-022-02664-4) [11]) and compresses to MFASS auPRC 0.54. AlphaGenome achieved SOTA on 25 of 26 vendor variant-effect benchmarks with splicing auPRCs in the 0.51–0.66 range across ClinVar and MFASS subsets ([Avsec et al., 2026, *Nature*](https://www.nature.com/articles/s41586-025-10014-0) [23]); an independent evaluation by Lu et al. reported AUROC 0.592 using the vendor's cross-modality scorer versus 0.779 using the best splicing-specific scorer ([Holgersen review of Lu et al., 2025](http://erle.bio/blog/2025-11-08-how-robust-are-clinvar-variant-subsets/) [25]) — a compression pattern that our own Rule A versus Rule B contrast mechanistically extends.

The *CFTR*-specific result adds a new observation to this literature: on an external, quantitative, pre-registered clinical-locus benchmark, three architecturally distinct predictors converge to a shared discrimination ceiling. On the binary-outcome Stratum 2 the three predictors sit within 0.02 AUC of each other under either AlphaGenome scoring rule, and the paired DeLong differences sit inside their SE bands. This convergence, together with the between-rule instability documented above, suggests that on external clinical quantitative benchmarks the current generation of splice predictors have reached a shared performance ceiling determined by the intrinsic difficulty of the task on canonical splice-site variants rather than by model capability, and that at this ceiling scoring-rule choice — not model choice — is the dominant source of apparent performance variation. This pattern would need to be tested on deep-intronic and exonic-splicing-enhancer variants, where all three predictors are known to be less accurate, before it could be generalised beyond the variant distribution represented here.
...
23. https://www.nature.com/articles/s41586-025-10014-0
24. https://pure.eur.nl/ws/files/218854365/1-s2.0-S2666247725001241-main.pdf
25. http://erle.bio/blog/2025-11-08-how-robust-are-clinvar-variant-subsets/
